...
%% Bus data creation
bus_headers = {'index', 'name', 'slack', 'gs_mw', 'bs_mvar', 'vn_kv', 'v_pu_min', 'v_pu_max'};
...
bus_data = [bus_i bus_name bus_slack gs_mw bs_mvar mpc.bus(:,[BASE_KV VMIN VMAX])];

%% Generators data creation
gen_headers = {'index', 'name', 'bus', 'p_mw_ini', 'p_mw', 'q_mvar', 'p_mw_min', 'p_mw_max', 'q_mvar_min', 'q_mvar_max', 'is_active', 'v_pu_set', 'c2', 'c1', 'controllable', 'priority', 's_max_mode', 'd_mw_max'};
...
gen_data = [gen_i gen_name gen_bus p_mw_ini mpc.gen(:,[PG QG PMIN PMAX QMIN QMAX GEN_STATUS]) v_set c_2 c_1 controllable priority s_max_mode d_mw_max];

%% Loads data creation
load_headers = {'index', 'name', 'bus', 'load_type', 'is_active', 'controllable', 'priority', 'p_mw', 'q_mvar', 's_mode', 'p_mw_max', 'p_mw_min', 'q_mvar_max', 'q_mvar_min', 'flex_price', 'flex_price_mode'};
load_i = (0:(sum(mpc.bus(:, PD) ~= 0) - 1)).';
load_name = "lcrit" + load_i;
load_bus = ((mpc.bus(:, PD) ~= 0).*mpc.bus(:, BUS_I));
load_bus(load_bus == 0) = [];
load_bus = load_bus -1;
if any((mpc.bus(:, PD) ~= 0) - (mpc.bus(:, QD) ~= 0))
...
load_data = [load_i load_name load_bus load_type is_active controllable priority p_mw q_mvar s_mode p_mw_max p_mw_min q_mvar_max q_mvar_min	flex_price flex_price_mode];

%% Branchs data creation
line_headers = {'index', 'name', 'from_bus', 'to_bus', 's_mva_max', 'is_active', 'r_pu', 'x_pu', 'b_sh_pu', 'ang_grad_min', 'ang_grad_max', 'overload_pu'};
...
line_data = [line_i line_name from_bus to_bus s_max mpc.branch(:,[BR_STATUS BR_R BR_X BR_B ANGMIN ANGMAX]) overload_pu];
